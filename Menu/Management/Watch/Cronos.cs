    ...
    public class Cronos
    {
        ...
        public static string Readjust(string text, int characters, int amount)
        {
            ...
            if (scroll_index + characters < text.Length && amount > 0)
                result += $"<color={CronosColorUtilities.Color32ToHTML(Preferences.text_accent)}>{new string('.', amount)}</color>";
        ...
        public static void Start()
        {
            ...
            pages.Add(new Button[]
            {
                ...
                new Button { title = "Follow Theme", status = Button.Statuses.Functional, toggled = false, isToggleable = true, blatant = false, tooltip = "Makes visual mods follow your color theme", action = () => FollowTheme.Update(true), disableAction = () => FollowTheme.Update(false) },
                new Button { title = "Ghost Rig", status = Button.Statuses.Functional, toggled = true, isToggleable = true, blatant = true, tooltip = "Adds a client-side rig when using rig mods", action = () => GhostRig.Update(true), disableAction = () => GhostRig.Update(false) },
            ...
            pages.Add(new Button[]
            {
                ...
                new Button { title = "Panic", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Disables all mods", action = () => Panic.Run() },
                new Button { title = "RPC Bypass", status = Button.Statuses.Unsafe, toggled = true, isToggleable = true, blatant = false, tooltip = "Removes RPC reports (Client side)" },
            ...
            pages.Add(new Button[]
            {
                ...
                new Button { title = "Box ESP", status = Button.Statuses.Functional, toggled = false, isToggleable = true, blatant = false, tooltip = "Makes hallow boxes around all players", action = () => BoxESP.Run(), disableAction = () => BoxESP.Cleanup() },
                new Button { title = "Distance Text", status = Button.Statuses.Functional, toggled = false, isToggleable = true, blatant = false, tooltip = "Makes distance counters on all players", action = () => DistanceText.Run(), disableAction = () => DistanceText.Cleanup() },
                ...
                new Button { title = "Nametags", status = Button.Statuses.Functional, toggled = false, isToggleable = true, isAdjustable = true, optionIndex = 0, optionTitle = "Mode", options = new string[] { "Simple", "Fancy" }, blatant = false, tooltip = "Creates nametags above all players", action = () => Nametags.Run(), disableAction = () => Nametags.Cleanup() },
                ...
                new Button { title = "Statistics HUD", status = Button.Statuses.Functional, toggled = false, isToggleable = true, blatant = false, tooltip = "Adds an interface that displays statistics", action = () => StatisticsHUD.Run(), disableAction = () => StatisticsHUD.Cleanup() },
                new Button { title = "Back", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Returns to the home page", action = () => UpdateCurrentPage(0) }
            });

            pages.Add(new Button[]
            {
                new Button { title = "Reach", status = Button.Statuses.Functional, toggled = false, isToggleable = true, isAdjustable = true, optionIndex = 0, optionTitle = "Amount", options = new string[] { "0.3", "0.4", "0.5", "0.6", "0.7", "0.8" }, blatant = false, tooltip = "Tag people from farther away", action = () => Reach.Run(), disableAction = () => Reach.Cleanup() },
            ...
            pages.Add(new Button[]
            {
                ...
                new Button { title = "Reset Soccer Game", status = Button.Statuses.Master, toggled = false, isToggleable = false, blatant = true, tooltip = "Resets the soccer score", action = () => ResetSoccerGame.Run() },
                new Button { title = "Set Soccer Score", status = Button.Statuses.Master, toggled = false, isToggleable = false, isAdjustable = true, optionIndex = 0, optionTitle = "Team", options = new string[] { "Red", "Blue" }, blatant = true, tooltip = "Adds a point to your specified team", action = () => SetSoccerScore.Run() },
                new Button { title = "Back", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Returns to the home page", action = () => UpdateCurrentPage(0) }
            });
            ...
            pages.Add(new Button[]
            {
                ...
                new Button { title = "Change Region", status = Button.Statuses.Functional, toggled = false, isToggleable = false, isAdjustable = true, optionIndex = 0, optionTitle = "Region", options = new string[] { "US", "USW", "EU" }, blatant = false, tooltip = "Changes your server region", action = () => ChangeRegion.Run() },
                new Button { title = "Join MOD", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Joins code MOD", action = () => PhotonNetworkController.Instance.AttemptToJoinSpecificRoom("MOD", JoinType.Solo) },
                new Button { title = "Join MODS", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Joins code MODS", action = () => PhotonNetworkController.Instance.AttemptToJoinSpecificRoom("MODS", JoinType.Solo) },
                new Button { title = "Back", status = Button.Statuses.Functional, toggled = false, isToggleable = false, blatant = false, tooltip = "Returns to the home page", action = () => UpdateCurrentPage(0) }
            });
        }
        ...
        public static void Load()
        {
            if (GorillaTagger.hasInstance)
            {
                if (Preferences.preferences[0])
                    theme = Color.Lerp(Preferences.theme[0], Preferences.theme[1], (Mathf.Sin(Time.time * Mathf.PI / 3f) + 1f) / 2f);
                else
                    if (theme != Preferences.theme[0])
                ...
                if (!initialized)
                {
                    ...
                    anchor.Find("Material").GetComponent<Image>().color = Preferences.toggles[2];
                    ...
                    if (authenticated)
                    {
                        ...
                        if (waked)
                        {
                            ...
                            else
                            {
                                string display = Readjust(buttons[mod_index].title, 11, 3) +
                                    $"{(buttons[mod_index].isAdjustable ? $"\n<color={CronosColorUtilities.Color32ToHTML(Preferences.text_accent)}>({buttons[mod_index].optionTitle}: {buttons[mod_index].options[buttons[mod_index].optionIndex]})</color>" : null)}" +
                                    $"{$"{(buttons[mod_index].isAdjustable ? "\n" : "\n\n")}{new string(' ', 5)}({mod_index + 1}/{buttons.Length})"}";
                                Color indicator = buttons[mod_index].status != Button.Statuses.Broken ? buttons[mod_index].isToggleable ? (buttons[mod_index].toggled ? Preferences.toggles[0] : Preferences.toggles[1]) : Preferences.toggles[2] : Color.black;
                        ...
                        if (ControllerInput.leftStick() && ControllerInput.rightStick())
                        {
                            if (!waked)
                            {
                                ...
                                GorillaTagger.Instance.offlineVRRig.PlayHandTapLocal(8, true, Preferences.volume);
                            ...
                            else
                            {
                                if (Time.time >= toggle_cooldown)
                                {
                                    ...
                                    GorillaTagger.Instance.offlineVRRig.PlayHandTapLocal(175, true, Preferences.volume);
                        ...
                        if (waked)
                        {
                                            ...
                                            if (Time.time >= mod_cooldown)
                                            {
                                                if (buttons[mod_index].status != Button.Statuses.Broken)
                                                {
                                                    if (buttons[mod_index].status == Button.Statuses.Unsafe)
                                                        Notifications.Send("NOTICE", $"{buttons[mod_index].title} modifies networking components, use at your own risk", Color.blue);
                                                    else if (buttons[mod_index].status == Button.Statuses.Master)
                                                        Notifications.Send("NOTICE", $"{buttons[mod_index].title} requires master client", Color.blue);

                                                    if (buttons[mod_index].isToggleable)
                                                    {
                                                        buttons[mod_index].toggled = !buttons[mod_index].toggled;
                                                        Notifications.Send("CRONOS", $"{buttons[mod_index].title}: {(buttons[mod_index].toggled ? "On" : "Off")}", Color.magenta);
                                                    ...
                                                    else
                                                    {
                                                        if (Preferences.preferences[1])
                                                        {
                                                            if (buttons[mod_index].blatant)
                                                                Notifications.Send("NOTICE", "Turn off Full Ghost Mode to use this mod", Color.blue);
                                                            else
                                                    ...
                                                    Notifications.Send("NOTICE", $"Cannot use {buttons[mod_index].title}, this mod is currently broken", Color.blue);
                            ...
                            text.text = "Cronos Cheat Watch" + $"\n<color={CronosColorUtilities.Color32ToHTML(Preferences.text_accent)}>{pages[0].Length - 1} categories\n{pages.Sum(page => page.Length)} mods</color>";
                    }
                    else
                    {
                        time += Time.deltaTime;
                        text.text = $"Loading...\n<color={CronosColorUtilities.Color32ToHTML(Preferences.text_accent)}>({Mathf.FloorToInt(time)} seconds elapsed)</color>";
                    }
                }
            }
        }
